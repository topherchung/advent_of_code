...
inputss = ["1364,461,1438,1456,818,999,105,1065,314,99,1353,148,837,590,404,123",
...
def calc_sschecksum(ss) -> int: # type: List[str]
    checksum = 0                # type:int
    for line in ss:             # type: str
        min = 999999            # type:int
        max = 0                 # type:int
        values = line.split(',')# type:List[str]
        for v in values:        # type: str
            v = int(v)
            if v < min:
                min = v
            if max < v:
...
def test_sschecksum():
    assert calc_sschecksum(testss) == 18, "Checksum should have equaled 18."


def calcEvenlyDivisableChecksum(ss) -> int:  # type: List[str]
    checksum = 0                             # type:int
    for line in ss:                          # type: str
        values = line.split(',')
        l = len(values)
        for i in range(l):                   # type: int
            for j in range(l):               # type: int
                if i == j:
                    continue
                vi = int(values[i])
                vj = int(values[j])
...
def test_DivSSChecksum():
    assert calcEvenlyDivisableChecksum(testDivSS) == 9, "Even Div Checksum should have equaled 9."

def main():
    test_sschecksum()
    test_DivSSChecksum()

    checksum = calc_sschecksum(inputss)
    print("Input spread sheet checksum = " + str(checksum))

    checksum = calcEvenlyDivisableChecksum(inputss)
    print("Input spread sheet evenly div checksum = " + str(checksum))
